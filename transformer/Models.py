...
class CRNN_Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.crnn = CNNLayer()
        self.bn = nn.BatchNorm1d(len_max_seq)
    ...
    def forward(self, src_seq, src_pos, image, image_padding, return_attns=False):
        ...
        non_pad_mask = get_non_pad_mask(seq_k)

        # -- Forward
        seq_emb = self.src_word_emb(src_seq) + self.position_enc(src_pos)
        image_emb = self.crnn(image).squeeze(2).permute(0, 2, 1)

        # concat CRNN and embedding
        enc_output = torch.cat((seq_emb, image_emb), 1)
        # Batch Normalization
